...
export default async function ProductsPage() {
  ...
  return <>
    <PageHeader title="Products" description="Manage BIGO products, USD cost basis, PHP selling price, and archive status." />
    <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
      <Card><h2 className="mb-4 font-semibold">Create product</h2><form action={createProduct} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3"><div><label>USD Cost</label><input name="unit_cost_usd" required step="0.01" type="number" /></div><div><label>PHP Price</label><input name="unit_price_php" required step="0.01" type="number" /></div></div>
